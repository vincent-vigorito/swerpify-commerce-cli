...
**customer-tags** — Manage customer tags
...
- `swerpicommerce-pp-cli vetrina product-datasheet-delete` — Azzera `scheda_pdf`; il file viene eliminato se nessun altro prodotto lo referenzia. Prodotto senza scheda -> 404...
- `swerpicommerce-pp-cli vetrina product-datasheet-upload` — Base64 nel body (`filename` .pdf + `content`) oppure `source: {folder, nome}` con un PDF già in libreria (cartella...
- `swerpicommerce-pp-cli vetrina product-delete` — Elimina record e riferimenti alle immagini (i file restano in libreria, possono essere condivisi: `DELETE /media`...
...
- `swerpicommerce-pp-cli vetrina products-list` — Ogni prodotto include `immagini` (ordinate, la prima è la principale), `categoria_percorso`, `url` (scheda...
