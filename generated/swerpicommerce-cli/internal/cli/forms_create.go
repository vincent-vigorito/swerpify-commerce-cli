...
func newFormsCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().BoolVar(&bodyAllegatiAttivi, "allegati-attivi", false, "Accetta campi file (`sw-form-file`) nelle submission.")
	cmd.Flags().IntVar(&bodyAllegatiMaxMb, "allegati-max-mb", 0, "Dimensione massima per file allegato, in MB. Default in creazione: 10")
	cmd.Flags().StringVar(&bodyAzioni, "azioni", "", "Azioni eseguite al submit, nell'ordine dato. Default in creazione: [{tipo: email}].")
