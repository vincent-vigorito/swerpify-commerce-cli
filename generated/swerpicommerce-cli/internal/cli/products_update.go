...
func newProductsUpdateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				if cmd.Flags().Changed("tab-extra") || bodyTabExtra != "" {
					...
					asArray, ok := parsedTabExtra.([]any)
					if !ok {
						return fmt.Errorf("--tab-extra must be a JSON array, got JSON %T", parsedTabExtra)
					}
					bodyMap["tab_extra"] = asArray
				}
	...
	cmd.Flags().StringVar(&bodyAlternates, "alternates", "", "Versioni multilingua collegate a questo prodotto, come dal pannello.")
	...
	cmd.Flags().StringVar(&bodyTabExtra, "tab-extra", "", "Contenuto di questo prodotto per i tab extra di tipo `specifico` (solo in update).")
