...
func newCustomAppsCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create",
		Short:       "Scaffolda la app, la registra (INSTALLED_APPS + rotte + menu)",
		Annotations: map[string]string{"pp:endpoint": "custom-apps.create", "pp:method": "POST", "pp:path": "/custom-apps", "pp:requires-input": "true"},
	...
	cmd.Flags().BoolVar(&bodyAdopt, "adopt", false, "Registra nel registro un'app che esiste GIA' su disco (nata a mano, es.")
	...
	cmd.Flags().BoolVar(&bodyMountAdmin, "mount-admin", true, "Monta `urls.py` sotto `/sw-back/<name>/` e crea AUTOMATICAMENTE la voce di menu (label/icon).")
	cmd.Flags().BoolVar(&bodyMountFrontend, "mount-frontend", false, "Monta `frontend_urls.py` a top-level (rotte PUBBLICHE `/<name>/`). Richiede un `frontend_urls.")
