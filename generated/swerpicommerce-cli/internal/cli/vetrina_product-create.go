...
func newVetrinaProductCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "product-create",
		Short:       "Se `slug` manca viene generato dal nome; è unico per lingua (suffisso `-2`, `-3` in caso di conflitto).",
		Annotations: map[string]string{"pp:endpoint": "vetrina.product-create", "pp:method": "POST", "pp:path": "/vetrina/products", "pp:requires-input": "true"},
