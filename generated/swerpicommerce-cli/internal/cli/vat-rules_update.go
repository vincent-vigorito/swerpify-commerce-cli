...
func newVatRulesUpdateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "update",
		Short:       "I campi omessi restano invariati.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				body = bodyMap
				if cmd.Flags().Changed("attive") {
					bodyMap["attive"] = bodyAttive
				}
				if cmd.Flags().Changed("nazione-azienda") || bodyNazioneAzienda != "" {
					bodyMap["nazione_azienda"] = bodyNazioneAzienda
				}
				if cmd.Flags().Changed("reverse-charge-attivo") {
					bodyMap["reverse_charge_attivo"] = bodyReverseChargeAttivo
				}
				if cmd.Flags().Changed("territori-esclusi-attivi") {
					bodyMap["territori_esclusi_attivi"] = bodyTerritoriEsclusiAttivi
				}
				if cmd.Flags().Changed("vies-attivo") {
					bodyMap["vies_attivo"] = bodyViesAttivo
				}
				if cmd.Flags().Changed("vies-fallback") || bodyViesFallback != "" {
					bodyMap["vies_fallback"] = bodyViesFallback
				}
			}
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "vat-rules", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "vat-rules", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "vat-rules", partialFailure.Message))
						}
						return nil
					}
				} else {
