...
func newMaintenanceUpdateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "update",
		Short:       "Aggiornamento parziale: si toccano solo le modalità presenti nel body (e, dentro ciascuna, solo le chiavi presenti).",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if stdinBody {
				...
				body = bodyMap
				{
					...
					if cmd.Flags().Changed("manutenzione-attiva") {
						nestedManutenzione["attiva"] = bodyManutenzioneAttiva
					}
					if cmd.Flags().Changed("manutenzione-messaggio") || bodyManutenzioneMessaggio != "" {
						nestedManutenzione["messaggio"] = bodyManutenzioneMessaggio
					}
					if len(nestedManutenzione) > 0 {
						bodyMap["manutenzione"] = nestedManutenzione
					}
				}
				{
					nestedVacanza := map[string]any{}
					if cmd.Flags().Changed("vacanza-attiva") {
						nestedVacanza["attiva"] = bodyVacanzaAttiva
					}
					if cmd.Flags().Changed("vacanza-messaggio") || bodyVacanzaMessaggio != "" {
						nestedVacanza["messaggio"] = bodyVacanzaMessaggio
					}
					if len(nestedVacanza) > 0 {
						bodyMap["vacanza"] = nestedVacanza
					}
				}
			}
