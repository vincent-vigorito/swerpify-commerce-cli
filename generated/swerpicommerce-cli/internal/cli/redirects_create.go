...
func newRedirectsCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "create",
		Short:       "La regola e' attiva subito (rigenera e ricarica nginx).",
		Annotations: map[string]string{"pp:endpoint": "redirects.create", "pp:method": "POST", "pp:path": "/redirects", "pp:requires-input": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("destinazione") && bodyDestinazione == "" && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "destinazione")
				}
				if !cmd.Flags().Changed("nome") && bodyNome == "" && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "nome")
				}
				if !cmd.Flags().Changed("origine") && bodyOrigine == "" && !flags.dryRun {
					return fmt.Errorf("required flag \"%s\" not set", "origine")
				}
			}
			path := "/redirects"
			...
			if stdinBody {
				...
				body = bodyMap
				if cmd.Flags().Changed("destinazione") || bodyDestinazione != "" {
					bodyMap["destinazione"] = bodyDestinazione
				}
				if cmd.Flags().Changed("nome") || bodyNome != "" {
					bodyMap["nome"] = bodyNome
				}
				if cmd.Flags().Changed("origine") || bodyOrigine != "" {
					bodyMap["origine"] = bodyOrigine
				}
				if cmd.Flags().Changed("origine-tipo") || bodyOrigineTipo != "" {
					bodyMap["origine_tipo"] = bodyOrigineTipo
				}
				if cmd.Flags().Changed("status-code") || bodyStatusCode != 0 {
					bodyMap["status_code"] = bodyStatusCode
				}
			}
