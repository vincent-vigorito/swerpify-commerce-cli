...
func newMediaUpdateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "`alt` viene salvato in libreria e propagato agli usi correnti del file (foto prodotto, `immagine_alt` delle categorie).",
			...
			if len(args) == 0 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s%s", cmd.CommandPath(), " <folder> <filename>"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("missing required argument\nUsage: %s%s", cmd.CommandPath(), " <folder> <filename>"))
			}
			if !stdinBody {
			}
			path := "/media/{folder}/{filename}"
			if len(args) < 1 || args[0] == "" {
				return usageErr(fmt.Errorf("folder is required\nUsage: %s <%s>", cmd.CommandPath(), "folder"))
			}
