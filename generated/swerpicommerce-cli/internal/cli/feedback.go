...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".swerpicommerce-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("SWERPICOMMERCE_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("SWERPICOMMERCE_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "swerpicommerce-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.swerpicommerce-pp-cli/feedback.jsonl.
When ` + "`SWERPICOMMERCE_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`SWERPICOMMERCE_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  swerpicommerce-pp-cli feedback list
  swerpicommerce-pp-cli feedback list --limit 5
  swerpicommerce-pp-cli feedback list --json`,
