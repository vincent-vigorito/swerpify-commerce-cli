...
func newWebhooksDeliveriesWebhookListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:         "webhook-list <id>",
		Aliases:     []string{"get"},
		Short:       "Ultime consegne tentate, dalla più recente.",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cmd.Flags().Changed("evento") {
				...
				if !validEvento {
					return fmt.Errorf("invalid value %q for --%s: must be one of %v", flagEvento, "evento", allowedEvento)
				}
			}
			...
			data, prov, err := resolvePaginatedReadWithStrategy(cmd.Context(), c, flags, "live", "deliveries", path, retainCLIQueryParams(cmd, map[string]string{
				"evento": formatCLIParamValue(flagEvento),
				"limit":  formatCLIParamValue(flagLimit),
				"offset": formatCLIParamValue(flagOffset),
			}, map[string][]string{"evento": {"evento"}, "limit": {"limit"}, "offset": {"offset"}}, "offset", "offset"), nil, flagAll, "offset", "offset", "limit", 100, "", "", "data", cmd.ErrOrStderr())
			if err != nil {
