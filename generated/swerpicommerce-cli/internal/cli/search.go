...
In local mode: searches locally synced data only.`,
  ...
  swerpicommerce-pp-cli search "status" --data-source local
  # Search a specific resource type locally
  swerpicommerce-pp-cli search "status" --type article-categories --data-source local
  ...
  swerpicommerce-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if flags.dataSource != "local" {
				...
				data, getErr := c.Get(cmd.Context(), "/fork/search", map[string]string{
					"q": query,
				})
				if getErr == nil {
