...
func newFormsSubmissionsFormReplayCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Rigioca le azioni configurate sugli `inputs` gia' registrati nel log",
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) == 0 {
				...
				if flags.asJSON {
					if printErr := printJSONFiltered(cmd.OutOrStdout(), map[string]any{
						...
						"usage": fmt.Sprintf("%s%s", cmd.CommandPath(), " <id> <submission_id>"),
					}, flags); printErr != nil {
						return printErr
					}
				}
				return usageErr(fmt.Errorf("missing required argument\nUsage: %s%s", cmd.CommandPath(), " <id> <submission_id>"))
			}
			if !stdinBody {
			}
			...
			if stdinBody {
				...
				body = bodyMap
				if cmd.Flags().Changed("azioni") {
					parsedAzioni, parseErr := cliutil.ParseStringList(bodyAzioni)
					if parseErr != nil {
						return fmt.Errorf("parsing --azioni list: %w", parseErr)
					}
					bodyMap["azioni"] = parsedAzioni
				}
