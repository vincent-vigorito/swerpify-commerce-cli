...
func newPageTemplatesAssignCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Scrive `PagineSistema.nome_file` (stessa cosa del pannello /sw-back/setting/grafica).",
		Annotations: map[string]string{"pp:endpoint": "page-templates.assign", "pp:method": "PUT", "pp:path": "/page-templates/{tipo}", "pp:requires-input": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !stdinBody {
				if !cmd.Flags().Changed("nome-file") && bodyNomeFile == "" && !flags.dryRun {
			...
			path = replacePathParam(path, "tipo", formatCLIParamValue(flagTipo))
			...
			if stdinBody {
				...
				body = bodyMap
				if cmd.Flags().Changed("nome-file") || bodyNomeFile != "" {
					bodyMap["nome_file"] = bodyNomeFile
				}
			}
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "page-templates", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "page-templates", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "page-templates", partialFailure.Message))
						}
						return nil
					}
				} else {
