...
func newEmailListsCreateCmd(flags *rootFlags) *cobra.Command {
	var bodyAttivo bool
	...
	cmd := &cobra.Command{
		Use:         "create",
		Short:       "Crea una lista email",
		Annotations: map[string]string{"pp:endpoint": "email-lists.create", "pp:method": "POST", "pp:path": "/email-lists", "pp:requires-input": "true"},
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 {
				...
				if partialFailure != nil {
					fmt.Fprintf(os.Stderr, "warning: partial failure detected in %s response: %s\n", "email-lists", partialFailure.Message)
			...
			if !flags.dryRun && statusCode >= 200 && statusCode < 300 && (partialFailure == nil || flags.allowPartialFailure) {
				writeMutationResponseToStore(cmd.Context(), "email-lists", data, "")
			}
				...
				if json.Unmarshal(data, &items) == nil && len(items) > 0 {
					if err := printAutoTable(cmd.OutOrStdout(), items); err != nil {
						...
						if partialFailure != nil && !flags.allowPartialFailure {
							return partialFailureErr(fmt.Errorf("partial failure in %s response: %s", "email-lists", partialFailure.Message))
						}
						return nil
					}
				} else {
