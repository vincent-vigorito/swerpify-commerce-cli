...
func newForkCommitCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:       "Stagea l'INTERO working tree (`git add -A`), bumpa `fork_version.json` (la versione fork: MAI toccare `version.",
		Annotations: map[string]string{"pp:endpoint": "fork.commit", "pp:method": "POST", "pp:path": "/fork/commit", "pp:requires-input": "true"},
	...
	cmd.Flags().BoolVar(&bodyForce, "force", false, "DISTRUTTIVO: se true, il push usa `--force` e SOVRASCRIVE la history remota di origin/<branch>")
