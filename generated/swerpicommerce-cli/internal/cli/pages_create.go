...
func newPagesCreateCmd(flags *rootFlags) *cobra.Command {
	...
	cmd.Flags().IntVar(&bodyPaginaPadreId, "pagina-padre-id", 0, "Id della pagina padre: è così che si fanno le gerarchie (l'URL pubblico diventa /<slug-padre>/<slug>/), non...")
	cmd.Flags().BoolVar(&bodySitemap, "sitemap", false, "Sitemap")
	cmd.Flags().StringVar(&bodySlug, "slug", "", "Se assente viene generato dal titolo (univoco per lingua). Solo lettere, numeri, '-' e '_': niente '/' — per le...")
