...
func RegisterTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcplib.NewTool("products_create",
			mcplib.WithDescription("**Guard anti-duplicato:** se il body ha un `sku` non vuoto e esiste già un prodotto con lo stesso `sku` nella stessa `lang`, risponde **409 `PRODUCT_DUPLICATE_SKU`** (con l'`id` esistente) invece di creare un doppione — usa `PUT /products/{id}` per aggiornarlo. Lo stesso `sku` su lingue diverse è invece consentito (ogni traduzione è un prodotto separato). **Variazioni prodotto:** prima crea (o assicura) gli attributi e i valori nel registro dal pannello (`GET /attributes` per enumerarli), poi crea il padre con `tipo_prodotto: variabile` e infine una variazione per combinazione con `tipo_prodotto: variante`, `prod_principale_id` del padre e `valori_attributi` (es. `[{'attributo': 'Taglia', 'valore': 'S'}]`, risolti contro il registro — match esatto case-sensitive, coppie non risolvibili -> **400 `VALIDATION_ERROR`**). La variante eredita lo slug del padre (niente scheda autonoma) e compare nel selettore della scheda del padre; una combinazione già esistente sullo stesso padre risponde **409 `PRODUCT_DUPLICATE_VARIANT`**. Required: nome. Optional: altezza, categoria_principale_id, categorie (plus 36 more). Returns the new ProductsCreateResponse."),
