...
func RegisterTools(s *server.MCPServer) {
	...
	s.AddTool(
		mcplib.NewTool("automation-settings_get",
			mcplib.WithDescription("Impostazioni delle automazioni. Returns the AutomationSettingsGetResponse."),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/automation-settings", []mcpParamBinding{}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("automation-settings_update",
			mcplib.WithDescription("Aggiorna le impostazioni. Required: retention_giorni. Returns the updated AutomationSettingsUpdateResponse."),
			mcplib.WithString("retention_giorni", mcplib.Required(), mcplib.Description("Retention giorni")),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("PUT", "/automation-settings", []mcpParamBinding{{PublicName: "retention_giorni", WireName: "retention_giorni", Location: "body"}}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("automations_create",
			mcplib.WithDescription("Nasce in `bozza` salvo `stato` esplicito; per attivarla serve almeno un nodo nel flusso. Il flusso viene validato (tipi di nodo, azioni, condizioni, attese) -> 400 `INVALID_FLOW` con il motivo. Required: nome, trigger_evento. Optional: flusso, stato, trigger_filtri. Returns the new AutomationsCreateResponse."),
			mcplib.WithString("flusso", mcplib.Description("Grafo del flusso: catena ordinata di nodi (il nodo «Fine» è implicito)")),
			mcplib.WithString("nome", mcplib.Required(), mcplib.Description("Nome")),
			mcplib.WithString("stato", mcplib.Description("`bozza` mai attivata; `attiva`; `in_pausa` spenta dall'utente; `errore` attiva ma l'ultima esecuzione è fallita...")),
			mcplib.WithString("trigger_evento", mcplib.Required(), mcplib.Description("Evento della piattaforma che scatena l'automazione. `customer.created`, `customer.updated`, `order.created`,...")),
			mcplib.WithString("trigger_filtri", mcplib.Description("Trigger filtri")),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("POST", "/automations", []mcpParamBinding{{PublicName: "flusso", WireName: "flusso", Location: "body"}, {PublicName: "nome", WireName: "nome", Location: "body"}, {PublicName: "stato", WireName: "stato", Location: "body"}, {PublicName: "trigger_evento", WireName: "trigger_evento", Location: "body"}, {PublicName: "trigger_filtri", WireName: "trigger_filtri", Location: "body"}}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("automations_delete",
			mcplib.WithDescription("Elimina anche il log delle esecuzioni e le email tracciate collegate. Required: id. Returns the AutomationsDeleteResponse. Destructive."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithDestructiveHintAnnotation(true),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("DELETE", "/automations/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("automations_get",
			mcplib.WithDescription("Dettaglio automazione (con flusso). Required: id. Returns the AutomationsGetResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/automations/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("automations_list",
			mcplib.WithDescription("Ogni automazione con contatori (esecuzioni, completate, errori, oggi, tasso di successo, ultima esecuzione) e il riassunto del flusso (`riassunto`, i chip del pannello). Il grafo completo (`flusso`) è nel dettaglio. Optional: stato, trigger_evento, limit (default: 100) (plus 1 more). Returns array of Automation."),
			mcplib.WithString("stato", mcplib.Description("Stato")),
			mcplib.WithString("trigger_evento", mcplib.Description("Trigger evento")),
			mcplib.WithString("limit", mcplib.Description("Numero massimo di risultati (default 100)")),
			mcplib.WithString("offset", mcplib.Description("Offset di paginazione (default 0)")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
		...
		makeAPIHandler("GET", "/automations", []mcpParamBinding{{PublicName: "stato", WireName: "stato", Location: "query"}, {PublicName: "trigger_evento", WireName: "trigger_evento", Location: "query"}, {PublicName: "limit", WireName: "limit", Location: "query"}, {PublicName: "offset", WireName: "offset", Location: "query"}}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("automations_lookups-get",
			mcplib.WithDescription("Eventi trigger (con i campi del payload filtrabili), tipi di azione e di condizione, operatori, unità di attesa, campi cliente aggiornabili, template email, tag, listini, liste email, lingue e segnaposto disponibili nei testi. È l'elenco che l'editor del pannello usa per le sue select. Returns the AutomationsLookupsGetResponse."),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/automations/lookups", []mcpParamBinding{}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("automations_update",
			mcplib.WithDescription("Campi parziali. `stato: attiva` richiede un flusso con almeno un nodo; `in_pausa` ferma i nuovi avvii (le esecuzioni in attesa vengono annullate al risveglio). `flusso` sostituisce l'intero grafo. Required: id. Optional: flusso, nome, stato (plus 2 more). Returns the updated AutomationsUpdateResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("flusso", mcplib.Description("Grafo del flusso: catena ordinata di nodi (il nodo «Fine» è implicito)")),
			mcplib.WithString("nome", mcplib.Description("Nome")),
			mcplib.WithString("stato", mcplib.Description("`attiva` richiede almeno un nodo; `in_pausa` ferma i nuovi avvii")),
			mcplib.WithString("trigger_evento", mcplib.Description("Evento della piattaforma che scatena l'automazione. `customer.created`, `customer.updated`, `order.created`,...")),
			mcplib.WithString("trigger_filtri", mcplib.Description("Trigger filtri")),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("PUT", "/automations/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "flusso", WireName: "flusso", Location: "body"}, {PublicName: "nome", WireName: "nome", Location: "body"}, {PublicName: "stato", WireName: "stato", Location: "body"}, {PublicName: "trigger_evento", WireName: "trigger_evento", Location: "body"}, {PublicName: "trigger_filtri", WireName: "trigger_filtri", Location: "body"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("automations_executions_automation-list",
			mcplib.WithDescription("Dalla più recente. Ogni esecuzione porta l'entità che l'ha scatenata, lo stato (`in_corso`, `in_attesa`, `completata`, `errore`, `annullata`), l'errore e i `passi` (nodo per nodo, con esito e dettaglio). Le esecuzioni chiuse vengono cancellate dopo `retention_giorni` (vedi `/automation-settings`). Required: id. Optional: stato, limit (default: 100), offset (default: 0). Returns array of AutomationExecution."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("stato", mcplib.Description("Stato")),
			mcplib.WithString("limit", mcplib.Description("Numero massimo di risultati (default 100)")),
			mcplib.WithString("offset", mcplib.Description("Offset di paginazione (default 0)")),
		...
		makeAPIHandler("GET", "/automations/{id}/executions", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "stato", WireName: "stato", Location: "query"}, {PublicName: "limit", WireName: "limit", Location: "query"}, {PublicName: "offset", WireName: "offset", Location: "query"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("automations_run_automation",
			mcplib.WithDescription("Esecuzione reale per un cliente (`cliente_id`: il payload viene costruito con la forma dell'evento trigger e i dati del cliente) o con un `payload` esplicito nella forma dell'evento (vedi `GET /webhook/openapi`). I filtri del trigger vengono applicati: se non passano risponde 200 con `data: null`. Le attese sono reali. Required: id. Optional: cliente_id, payload. Returns the new RunAutomationResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("cliente_id", mcplib.Description("Cliente per cui costruire il payload dell'evento trigger")),
			mcplib.WithString("payload", mcplib.Description("Payload esplicito nella forma dell'evento trigger (in alternativa a cliente_id)")),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("POST", "/automations/{id}/run", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "cliente_id", WireName: "cliente_id", Location: "body"}, {PublicName: "payload", WireName: "payload", Location: "body"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("automations_test_automation",
			mcplib.WithDescription("Come il tasto «Test» dell'editor: le email vengono inviate davvero ma solo a `email`; webhook, tag e campi sono simulati; le attese vengono saltate; le condizioni sull'email seguono il ramo SI. Con `flusso` si prova un grafo non ancora salvato. L'esecuzione è marcata `test: true` e non entra nelle statistiche. Required: id, email. Optional: flusso. Returns the new TestAutomationResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("email", mcplib.Required(), mcplib.Description("Unico destinatario delle email di prova")),
			mcplib.WithString("flusso", mcplib.Description("Grafo del flusso: catena ordinata di nodi (il nodo «Fine» è implicito)")),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("POST", "/automations/{id}/test", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "email", WireName: "email", Location: "body"}, {PublicName: "flusso", WireName: "flusso", Location: "body"}}, []string{"id"}),
	)
	s.AddTool(
	...
	s.AddTool(
		mcplib.NewTool("customer-tags_create",
			mcplib.WithDescription("Nome case-insensitive e univoco: se esiste già risponde 200 con il tag esistente. Required: nome. Optional: colore. Returns the new CustomerTagsCreateResponse."),
			mcplib.WithString("colore", mcplib.Description("Colore")),
			mcplib.WithString("nome", mcplib.Required(), mcplib.Description("Normalizzato in minuscolo")),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("POST", "/customer-tags", []mcpParamBinding{{PublicName: "colore", WireName: "colore", Location: "body"}, {PublicName: "nome", WireName: "nome", Location: "body"}}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("customer-tags_delete",
			mcplib.WithDescription("Elimina un tag da tutti i clienti. Required: id. Returns the CustomerTagsDeleteResponse. Destructive."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithDestructiveHintAnnotation(true),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("DELETE", "/customer-tags/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("customer-tags_list",
			mcplib.WithDescription("Ogni tag con il numero di clienti che lo hanno. Returns array of CustomerTagsListItem."),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/customer-tags", []mcpParamBinding{}, []string{}),
	)
	s.AddTool(
	...
	s.AddTool(
		mcplib.NewTool("customers_tags_customer-add",
			mcplib.WithDescription("Il tag viene creato se non esiste (nome case-insensitive). Idempotente. Required: id, nome. Optional: colore. Returns array of TagsCustomerAddItem."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("colore", mcplib.Description("Colore")),
			mcplib.WithString("nome", mcplib.Required(), mcplib.Description("Normalizzato in minuscolo")),
			mcplib.WithDestructiveHintAnnotation(false),
		...
		makeAPIHandler("POST", "/customers/{id}/tags", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "colore", WireName: "colore", Location: "body"}, {PublicName: "nome", WireName: "nome", Location: "body"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("customers_tags_customer-remove",
			mcplib.WithDescription("Il tag resta nel catalogo (`DELETE /customer-tags/{id}` per eliminarlo ovunque). Required: id, nome. Returns array of TagsCustomerRemoveItem. Destructive."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("nome", mcplib.Required(), mcplib.Description("Nome")),
			mcplib.WithDestructiveHintAnnotation(true),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("DELETE", "/customers/{id}/tags/{nome}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "nome", WireName: "nome", Location: "path"}}, []string{"id", "nome"}),
	)
	s.AddTool(
	...
	s.AddTool(
		mcplib.NewTool("vetrina_categories-list",
			...
			mcplib.WithString("lang", mcplib.Description("Filtra per lingua (match esatto). Omesso = tutte le lingue.")),
			mcplib.WithString("slug", mcplib.Description("Slug")),
			mcplib.WithString("categoria_padre_id", mcplib.Description("Figlie dirette di questa categoria; `0` = solo le categorie di primo livello.")),
			mcplib.WithString("attiva", mcplib.Description("Attiva")),
			mcplib.WithString("limit", mcplib.Description("Numero massimo di risultati (default 100)")),
			mcplib.WithString("offset", mcplib.Description("Offset di paginazione (default 0)")),
			mcplib.WithReadOnlyHintAnnotation(true),
		...
		makeAPIHandler("GET", "/vetrina/categories", []mcpParamBinding{{PublicName: "lang", WireName: "lang", Location: "query"}, {PublicName: "slug", WireName: "slug", Location: "query"}, {PublicName: "categoria_padre_id", WireName: "categoria_padre_id", Location: "query"}, {PublicName: "attiva", WireName: "attiva", Location: "query"}, {PublicName: "limit", WireName: "limit", Location: "query"}, {PublicName: "offset", WireName: "offset", Location: "query"}}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("vetrina_category-create",
			...
			mcplib.WithString("description", mcplib.Description("Meta description SEO")),
			mcplib.WithString("descrizione", mcplib.Description("HTML mostrato nella pagina della categoria, sotto la banda titolo. Indentalo in modo ordinato e leggibile.")),
			mcplib.WithString("descrizione_breve", mcplib.Description("Una riga di testo semplice: nelle card della radice del catalogo e nella banda titolo della categoria.")),
			mcplib.WithString("follow", mcplib.Description("Default in creazione: true.")),
			...
			mcplib.WithString("meta_title", mcplib.Description("Titolo SEO della pagina categoria")),
			mcplib.WithString("nome", mcplib.Required(), mcplib.Description("Nome")),
			mcplib.WithString("ordinamento", mcplib.Description("Posizione tra le sorelle. Default in creazione: 0.")),
	...
	s.AddTool(
		mcplib.NewTool("vetrina_category-delete",
			mcplib.WithDescription("Come dal pannello: le sottocategorie vengono eliminate in cascata, i prodotti restano senza categoria (`categoria_id: null`, raggiungibili a `/<pagina vetrina>/<slug>/`). Required: id. Returns the VetrinaCategoryDeleteResponse. Destructive."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithDestructiveHintAnnotation(true),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("DELETE", "/vetrina/categories/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("vetrina_category-get",
			mcplib.WithDescription("Dettaglio categoria vetrina. Required: id. Returns the VetrinaCategoryGetResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/vetrina/categories/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
	...
	s.AddTool(
		mcplib.NewTool("vetrina_product-create",
			...
			mcplib.WithString("index", mcplib.Description("Default in creazione: true.")),
			mcplib.WithString("keywords", mcplib.Description("Parole chiave SEO")),
			mcplib.WithString("lang", mcplib.Description("Codice lingua (es. it, en). Ogni traduzione è un prodotto separato. Default in creazione: lingua predefinita del sito.")),
	...
	s.AddTool(
		...
		makeAPIHandler("DELETE", "/vetrina/products/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("vetrina_product-get",
			mcplib.WithDescription("Dettaglio prodotto vetrina. Required: id. Returns the VetrinaProductGetResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithReadOnlyHintAnnotation(true),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("GET", "/vetrina/products/{id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
		mcplib.NewTool("vetrina_product-image-delete",
			mcplib.WithDescription("Rimuove solo il collegamento; il file resta in libreria (`DELETE /media` per eliminarlo). Required: id, image_id. Returns the VetrinaProductImageDeleteResponse. Destructive."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("image_id", mcplib.Required(), mcplib.Description("Image id")),
			mcplib.WithDestructiveHintAnnotation(true),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("DELETE", "/vetrina/products/{id}/images/{image_id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "image_id", WireName: "image_id", Location: "path"}}, []string{"id", "image_id"}),
	)
	s.AddTool(
		mcplib.NewTool("vetrina_product-image-update",
			mcplib.WithDescription("`principale: true` la porta in posizione 0 (le altre scalano). Campi non riconosciuti -> 400. Required: id, image_id. Optional: alt, posizione, principale. Returns the updated VetrinaProductImageUpdateResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("image_id", mcplib.Required(), mcplib.Description("Image id")),
			mcplib.WithString("alt", mcplib.Description("Testo alternativo dell'immagine (SEO/accessibilità)")),
			mcplib.WithString("posizione", mcplib.Description("Nuova posizione nella galleria (le altre scalano)")),
			mcplib.WithString("principale", mcplib.Description("true = porta l'immagine in posizione 0")),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("PUT", "/vetrina/products/{id}/images/{image_id}", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "image_id", WireName: "image_id", Location: "path"}, {PublicName: "alt", WireName: "alt", Location: "body"}, {PublicName: "posizione", WireName: "posizione", Location: "body"}, {PublicName: "principale", WireName: "principale", Location: "body"}}, []string{"id", "image_id"}),
	)
	s.AddTool(
		mcplib.NewTool("vetrina_product-image-upload",
			mcplib.WithDescription("Due modalità alternative: contenuto base64 nel body (`filename` + `content`, estensioni jpg/jpeg/png/webp/gif/avif/svg), oppure `source: {folder, nome}` con un file già in libreria (GET /media): se la cartella è `vetrina_prodotti` il file viene referenziato così com'è (come dal picker del pannello), altrimenti copiato in `/uploads/vetrina/prodotti_img/`. Con `source`, `alt` omesso eredita quello della libreria. `principale: true` la mette in testa (posizione 0); `posizione` la inserisce in quel punto; altrimenti va in coda. In caso di nome file già esistente lo storage lo rinomina: fa fede `nome` nella risposta. Required: id. Optional: alt, content, filename (plus 3 more). Returns the new VetrinaProductImageUploadResponse."),
			mcplib.WithString("id", mcplib.Required(), mcplib.Description("Id")),
			mcplib.WithString("alt", mcplib.Description("Testo alternativo dell'immagine (SEO/accessibilità)")),
			mcplib.WithString("content", mcplib.Description("Contenuto del file in base64")),
			mcplib.WithString("filename", mcplib.Description("Nome file con estensione (jpg/jpeg/png/webp/gif/avif/svg)")),
			mcplib.WithString("posizione", mcplib.Description("Posizione in cui inserirla (le successive scalano); omessa = in coda")),
			mcplib.WithString("principale", mcplib.Description("true = diventa la prima immagine (posizione 0)")),
			mcplib.WithString("source", mcplib.Description("File della libreria media (alternativo a filename+content): referenziato se in `vetrina_prodotti`, altrimenti copiato.")),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("POST", "/vetrina/products/{id}/images", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}, {PublicName: "alt", WireName: "alt", Location: "body"}, {PublicName: "content", WireName: "content", Location: "body"}, {PublicName: "filename", WireName: "filename", Location: "body"}, {PublicName: "posizione", WireName: "posizione", Location: "body"}, {PublicName: "principale", WireName: "principale", Location: "body"}, {PublicName: "source", WireName: "source", Location: "body"}}, []string{"id"}),
	)
	s.AddTool(
		...
		makeAPIHandler("GET", "/vetrina/products/{id}/images", []mcpParamBinding{{PublicName: "id", WireName: "id", Location: "path"}}, []string{"id"}),
	)
	s.AddTool(
	...
	s.AddTool(
		mcplib.NewTool("vetrina_products-batch",
			mcplib.WithDescription("Validazione di dominio per-item (vedi `errors[]`, con `index` e `nome`). Required: items. Returns the new VetrinaProductsBatchResponse."),
			mcplib.WithString("items", mcplib.Required(), mcplib.Description("Ogni item segue VetrinaProductInput; gli item non validi finiscono in errors[].")),
			mcplib.WithDestructiveHintAnnotation(false),
			mcplib.WithOpenWorldHintAnnotation(true),
		),
		makeAPIHandler("POST", "/vetrina/products/batch", []mcpParamBinding{{PublicName: "items", WireName: "items", Location: "body"}}, []string{}),
	)
	s.AddTool(
		mcplib.NewTool("vetrina_products-list",
			...
			mcplib.WithString("categoria_id", mcplib.Description("Prodotti della categoria; `0` = senza categoria.")),
