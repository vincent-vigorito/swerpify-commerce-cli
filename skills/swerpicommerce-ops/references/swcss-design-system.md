...
| `.sw-container` a 1280px e testo a 10 px dal bordo fra 1280 e ~1320 px di viewport, solo sulle pagine CMS | **B82**: sui tenant provisionati prima della 2.73 il `cms/layout.css` per-istanza (predefinito, auto-generato) ridefinisce `.sw-container` con `max-width: 1280px` e `padding-inline: 0` da `--xl`, e la sezione vince sul globale. L'hook 2.73.14 non lo tocca. Diagnosi: `design css-get layout.css --section cms` — se definisce `.sw-container` il tenant è colpito (03/09/2026: spnew, swerpifywebsite; puliti cosicome, detergenza, fresenium, bzchem). Non si patcha lato tenant (file predefinito): si aspetta l'hook di piattaforma |
