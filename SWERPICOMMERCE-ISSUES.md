...
| # | Problema | Stato |
|---|---|---|
| B79 | **Template `custombox-v2.html`: 18 delle 31 stringhe `{% blocktrans %}` non sono nei cataloghi `django.po` di en/es/fr → la custom box v2 esce mista IT/EN** — su spnew (122-h000672, 27/08, identico su 2.63.10 e su 2.66.18 dopo l'upgrade delle 12:29): dopo `PUT /page-templates/custom-box {"nome_file":"custombox-v2.html"}` la pagina EN `/en/negozio/tennis-strings/string-project-custom-box-of-9-12-m-sets/` mostra in italiano gli step 1-2 («Scegli i prodotti», «Sfoglia la selezione…», «Completa il box», «Componi il box con un minimo di…»), l'hero («Prezzo fisso:», «qualunque combinazione tu scelga»), il riepilogo («Mancano … articoli», «articoli aggiunti al box»), le card («Non disponibile», «Quantità insufficiente», «Solo … disponibili») e «punti»/«alberi»; le stringhe ereditate dalla v1 («Aggiungi al carrello», «Hai raggiunto il massimo…») sono tradotte. Stesso esito in ES (19/31) e FR (18/31). `django.po` è read-only e `custom.po` + `compila_locales.sh` non sono raggiungibili via API → nessun workaround lato tenant (solo fork del template con `custom_trans`, che però resta senza compilazione dei `.mo`). Richiesta: aggiungere le msgid del template v2 ai cataloghi upstream di tutte le lingue supportate e ricompilare i `.mo` nel rilascio. | 🔴 |
| B80 | **Restyle dell'area account (2.70.2, commit `798b1e88`) fatto nel solo layer `default/mio-account/`: i tenant già provisionati restano alla versione vecchia** — i preset per-istanza `mio_account/{variabili,componenti,tabs}.css` di Così Com'è (122-h000743, 2.70.2) sono **byte-identici al default PRE-restyle** (md5 uguali a `798b1e88~1`) e diversi dal nuovo default; il bundle live `account.css` serve ancora `--sw-stato-completato: #059669` invece di `var(--sw-success)`. Nessun `update_hooks/` accompagna il commit (a differenza di `2.61.18__css_form_base_campi_in_place.py` per B63). Impatto: il restyle esiste solo per i tenant creati da oggi; quelli vecchi non lo vedranno mai, e chi legge il default nel repo crede sia live. **Richiesta**: hook di aggiornamento che riscriva in place i preset `mio_account/*` non modificati dal tenant (stessa idempotenza del 2.61.18: no-op se il file è stato riscritto a mano), oppure documentare che i layer `pagine-sistema/` non seguono il default dopo il provisioning. **Workaround applicato su Così Com'è il 29/08**: i 5 file ricopiati dal default HEAD via `PUT /design/css/mio_account/<file>` + compile (backup dei precedenti conservato); `account.css` live ora con `--sw-stato-*` sui token della palette. Resta da fare sugli altri tenant esistenti finché non c'è l'hook. | 🔴 aperta (workaround su 122-h000743) |
| B81 | **I partial upstream `header_base.html`, `header_sticky_base.html`, `footer_base.html` non sono esposti dall'API, contro quanto promette `GET /design/templates-guide`** — la guida dice: «li puoi leggere (`GET`) come riferimento», «nel list sono nascosti di default: aggiungi `include_upstream=true`», e il flusso consigliato parte da «leggi il `*_base*`». In pratica `GET /design/templates?area=partials&include_upstream=true` elenca come upstream **solo** `breadcrumbs_blog`, `breadcrumbs_sistema`, `footer_carrello`, `minicart`, e `GET /design/templates/partials/header_base.html` → **404 `TEMPLATE_NOT_FOUND`** (idem sticky e footer). **Verificato 31/08/2026 su tre tenant** (122-h000743 cosicome 2.70.6, 122-h000749 fresenium, 122-h000744 detergenza), tutti con fork `header.html` presente. Impatto: chi deve rifare un header parte senza il riferimento canonico (hook JS load-bearing: `header_basic`, `menu_sticky`, `data-sw-side-*`, selettore lingua SSR) e senza poter vedere come l'upstream ha adottato le novità — es. oggi il tag `{% cart_icon %}` è verificabile solo su `pagamento.html`. **Richiesta**: esporre i tre `*_base*` in lettura (o dire nella guida quale file è il riferimento, se si chiama diversamente). | 🔴 aperta |
